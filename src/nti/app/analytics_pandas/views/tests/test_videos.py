...
class TestVideoOptions(ApplicationLayerTest):
    ...
    @WithSharedApplicationMockDS(testapp=True, users=True)
    def test_video_report_one_day(self):
        request = DummyRequest(params={'ntiid': 'tag:nextthought.com,2011-10:NTI-CourseInfo-Spring2015_SOC_1113',
        ...
        assert_that(options,
                    has_entries('data',
                                has_entries('video_events', is_not(none()),
                                            'video_events', 
                                            has_entries('num_rows_video_event_type', 2,
                                                        'by_video_event_type_chart', (),
                                                        'tuples_video_event_type', is_not(none()),
                                                        'num_rows', 1,
                                                        'events_chart', (),
                                                        'tuples', is_not(none()),
                                                        'column_name', u'Videos Watched',
